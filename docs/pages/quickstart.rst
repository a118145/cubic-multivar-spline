...
This section serves as quick demo how to use the library. The presented example can be found in the ``tests/demos`` directory of the `repository`_ inside :code:`demo_not-a-knot.py`.
...
.. literalinclude:: ../../tests/demos/demo_not-a-knot.py
   :start-after: #1s
   :end-before: #1e
...
The spline can be evaluated at any point in the domain using the :py:meth:`~cubicmultispline.Spline.eval_spline` method. To achieve a smooth surface, we evaluate the spline on a finer grid:
