...
from cubic_multivar_spline.SplineTest import eval_spline_ai
...
class Spline:
    ...
    #     res = 0
    #     for i in range(self._interval[0][2]+2):
    #         for j in range(self._interval[1][2]+2):
    ...
    """
    Multidimensional spline evaluation (vectorized)
    Based on: DOI 10.1007/s10614-007-9092-4, Section 3.2

    Requirements:
    - _phi(t)       : Basis function, vectorized, shape (...,)
    - _dphi_dt(t)   : 1. derivative
    - _d2phi_dt2(t) : 2. derivative
    - ci            : coefficients, reshape to (n1, n2, ..., nd) possible
    - knots         : List of 1D-Arrays, knots[k] = knot vector in dimension k

    Convention:
    - x has shape (N, d): N evaluation points, d dimensions
    - Return: (f, grad, hess)
        f    : (N,)
        grad : (N, d)
        hess : (N, d, d)
